...
// tee-node server and the veilbidding extension in a single process. Docker sets
...
import (
	...
	extserver "veilbidding/pkg/server"
)
...
func main() {
	...
	logger.Infof("veilbidding extension TEE running (config=%d, sign=%d, ext=%d)", configPort, signPort, extensionPort)
