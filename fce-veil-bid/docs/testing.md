...
**Policy buy mode:** `./scripts/test.sh` and `run-test` default to the **private** flow (`buyPolicyPrivate` + `relayPrivateBuy`). Use public `buyPolicy` instead:

```bash
cd tools && go run ./cmd/run-test/ -private=false \
  --instructionSender "$INSTRUCTION_SENDER" \
  -p "$EXT_PROXY_URL"
```

**Solidity unit tests** (`relayPrivateBuy` signature checks and `buyPolicyPrivate` instruction routing):
...
4. Poll for result            ← Generic: utils.ActionResult() polls proxy (20 retries, 2s apart; keeps polling while TEE status=2 “in processing”)
