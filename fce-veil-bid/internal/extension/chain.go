...
// minimalVeilBiddingABI covers only the two view functions the SCORE handler
// needs. The full contract ABI lives in tools/pkg/contracts/veilbidding, but
// that package belongs to the tools submodule (which depends on this one, not
// the other way around) — pulling it in here would require restructuring the
// module graph for two trivial getters.
