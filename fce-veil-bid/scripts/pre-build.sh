...
if [[ -f "$PROJECT_DIR/.env" ]]; then
    ...
    echo -e "${RED}[pre-build] ERROR:${NC} Missing $PROJECT_DIR/.env — run: ./scripts/use-chain.sh coston2  (or local coston2)" >&2
    exit 1
fi
...
# Must run before any `go run` — autogen.go is gitignored and produced here.
...
# --- Step 2.5: Verify contract source on the chain explorer (best-effort) ---
step 2.5 "Verify contract on explorer"
ADDRESSES_FILE="$ADDRESSES_FILE" CHAIN_URL="$CHAIN_URL" "$SCRIPT_DIR/verify-contract.sh" "$INSTRUCTION_SENDER" \
    || log "verification skipped/failed (non-fatal) — re-run later with ./scripts/verify-contract.sh $INSTRUCTION_SENDER"
