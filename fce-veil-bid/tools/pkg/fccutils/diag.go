...
func diagToProductionRevert(
	...
	if err != nil {
		logger.Warnf("diag: pack toProduction calldata: %v", err)
		return
	}
	...
	logger.Infof("diag: eth_call toProduction from=%s to=%s", from.Hex(), s.Addresses.FlareTeeManager.Hex())
	...
	for _, src := range []struct {
		...
		for errName, eDef := range a.Errors {
			...
			if unpackErr != nil {
				...
				logger.Errorf("diag: revert -> %s.%s%v", src.name, errName, args)
			}
			return
		}
	}
