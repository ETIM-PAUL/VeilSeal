...
// Scoped by contract address + schema version, same reasoning as
// LISTINGS_CACHE_KEY.
const CIPHER_LISTINGS_CACHE_KEY = `veilseal:cipher-listing-events-cache:v1:${VEIL_BIDDING_ADDRESS}`;
...
/// full word list, so no separate per-listing call is needed to render a
/// listing card. Returns [{ listingId, creator, deadline, wordCount, words,
/// txHash, blockNumber }], oldest first.
...
/// deadline, revealed/winner/wordCount, plus the settlement tx hash (from the
