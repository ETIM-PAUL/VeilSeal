...
// (title/description/type/ipfsHash/minBid/minScore) is stored on-chain — no
// off-chain index or localStorage cache required to render a listing.
const DEFAULT_ADDRESS = "0x69CCecabC4d8Eb025dE8a50F00A5eD6263fD9997";
...
export const DEPLOY_BLOCK = Number(import.meta.env.VITE_VEILBIDDING_DEPLOY_BLOCK || 33487267);

// The TEE's score formula (internal/extension/chain.go) caps at 100 — any
// minScore above that is mathematically unreachable by any wallet's score,
// so it turns a listing "invite only": bidding requires being on the
// participant allowlist, full stop, with no score escape hatch. No contract
// change needed for this — it's just a sentinel value.
export const MAX_SCORE = 100;
export const INVITE_ONLY_MIN_SCORE = MAX_SCORE + 1;

export function isInviteOnly(minScore) {
  return BigInt(minScore ?? 0) > BigInt(MAX_SCORE);
}

// Empty attestation — passed to submitSealedBid when a listing has no
// minScore gate, or the caller is an invited participant (bypasses the gate
